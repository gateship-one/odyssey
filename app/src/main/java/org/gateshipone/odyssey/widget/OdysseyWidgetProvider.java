...
OdysseyWidgetProvider extends AppWidgetProvider {
    ...
    private synchronized void setWidgetContent(@NonNull NowPlayingInformation info, Context context) {
        ...
        views.setTextViewText(R.id.widget_trackName, item.getTrackDisplayedName());
        views.setTextViewText(R.id.widget_artistAlbum, item.getTrackArtistName());
            ...
            case PAUSE: {
                if (!mHideArtwork) {
                    ...
                    if (!mLastInfo.getCurrentTrack().getTrackAlbumKey().equals(item.getTrackAlbumKey())) {
                        ...
                        views.setImageViewResource(R.id.widget_cover, R.drawable.odyssey_notification);
                        ...
                        views.setImageViewBitmap(R.id.widget_cover, mLastCover);
                    }
                } else {
        ...
        if (playState == PlaybackService.PLAYSTATE.PLAYING) {
            // Show pause icon
            nowPlaying = true;
            views.setImageViewResource(R.id.widget_play, R.drawable.ic_pause_48dp);
        } else {
            // Show play icon
            views.setImageViewResource(R.id.widget_play, R.drawable.ic_play_arrow_48dp);
        }
        ...
        views.setOnClickPendingIntent(R.id.widget_cover, mainPendingIntent);
