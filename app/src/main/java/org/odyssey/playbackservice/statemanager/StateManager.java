...
public class StateManager {
    ...
    public TrackModel getTrackModel(int id) {

        // get row id and return the TrackModel

        String whereVal[] = { "" + id };

        Cursor cursor = mPlaylistDB.query(SavedTracksTable.TABLE_NAME, projectionTrackModels, SavedTracksTable.COLUMN_ID + "=?", whereVal, "", "", SavedTracksTable.COLUMN_ID);
    ...
    public void enqueueTrackModel(TrackModel item) {

        // save TrackModel to database
    ...
    public void enqueueTrackList(ArrayList<TrackModel> list) {

        // save TrackModels to database

        ContentValues values = new ContentValues();

        mPlaylistDB.beginTransaction();

        for (TrackModel item : list) {
    ...
    public void dequeueTrackModel(int id) {
        ...
        mPlaylistDB.delete(SavedTracksTable.TABLE_NAME, SavedTracksTable.COLUMN_ID + "=?", whereVal);
    ...
    public void shufflePlaylist() {
        ...
        for (int i = 0; i < playList.size(); i++) {
            ...
            // set TrackModel parameters
            values.put(SavedTracksTable.COLUMN_TRACKTITLE, playList.get(i).getTrackName());
            values.put(SavedTracksTable.COLUMN_TRACKDURATION, playList.get(i).getTrackDuration());
            values.put(SavedTracksTable.COLUMN_TRACKNUMBER, playList.get(i).getTrackNumber());
            values.put(SavedTracksTable.COLUMN_TRACKARTIST, playList.get(i).getTrackArtistName());
            values.put(SavedTracksTable.COLUMN_TRACKALBUM, playList.get(i).getTrackAlbumName());
            values.put(SavedTracksTable.COLUMN_TRACKURL, playList.get(i).getTrackURL());
            values.put(SavedTracksTable.COLUMN_TRACKALBUMKEY, playList.get(i).getTrackAlbumKey());

            String whereVal[] = { "" + i };

            mPlaylistDB.update(SavedTracksTable.TABLE_NAME, values, SavedTracksTable.COLUMN_ID + "=?", whereVal);
        }
